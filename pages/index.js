...
export default function Home() {
	const { data } = useSWR('/api/tweets', (url) => fetch(url).then((res) => res.json()));
	...
	return (
		<>
			...
			<div className=''>
				{[...data.tweets].reverse().map((tweet, i) => (
					<div key={i} className='mx-2 px-2 py-3 border-b border-gray-200'>
						<div className='flex'>
							<img src={tweet.avatarUrl} className='w-10 h-10 mr-3 rounded-full' alt='' />
							<div className='flex flex-col min-w-0'>
								<p className='flex text-sm'>
									<span className='truncate text-cool-gray-500'>
										<span className='font-bold text-cool-gray-900'>{tweet.name} </span>
										<span className='pl-1 text-cool-gray-500'>@{tweet.userName}</span>
									</span>
									<span className='flex-shrink-0'>
										<span className='pl-1 text-cool-gray-500'>·</span>
